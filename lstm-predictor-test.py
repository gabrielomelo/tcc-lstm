...
class CoronaVirusPredictor(nn.Module):
    ...
    def __init__(self, n_features, n_hidden, seq_len, n_layers=2):
        ...
        self.lstm = nn.LSTM(input_size=n_features, hidden_size=n_hidden, num_layers=n_layers, dropout=0.5).cuda()
        self.linear = nn.Linear(in_features=n_hidden, out_features=1).cuda()
        self.hidden = None
    ...
    @staticmethod
    def train_model(_model, train_data, train_labels, test_data=None, test_labels=None, num_epochs=60, _lr=0.0001):
...
if __name__ == "__main__":
    ...
    X_all = torch.from_numpy(X_all).float().cuda()
    y_all = torch.from_numpy(y_all).float().cuda()
